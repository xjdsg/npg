...
func (conn *Conn) writeDescribe(stmt *Stmt) {
	msgLen := int32(4 + 1 + len(stmt.name) + 1)
...
func (conn *Conn) writeExecute(stmt *Stmt) {
	msgLen := int32(4 + len(stmt.name) + 1 + 4)
...
func (conn *Conn) writeParse(stmt *Stmt) {
	msgLen := int32(4 +
		len(stmt.name) + 1 +
		len(stmt.query) + 1 +
		2 + len(stmt.params)*4) //2 is the number of params

	conn.writeFrontendMessageCode(_Parse)
	conn.writeInt32(msgLen)
	conn.writeString0(stmt.name)
	conn.writeString0(stmt.query)

	conn.writeInt16(int16(len(stmt.params)))
	for _, param := range stmt.params {
		conn.writeInt32(param.ptype)
	}
